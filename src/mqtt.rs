use rocket::Rocket;
use chrono::Utc;
use jsonwebtoken::{encode, Algorithm, EncodingKey, Header};
use paho_mqtt::{
    Client, ConnectOptionsBuilder, CreateOptionsBuilder, Message, SslOptionsBuilder, SslVersion,
};
use serde::Serialize;
use std::fs::File;
use std::io::{BufReader, Read};
use std::sync::mpsc::{Receiver, TryRecvError};
use std::sync::{Arc, Mutex};
use std::thread;
use std::time::Duration;

use crate::models::command::GatewayCommand;
use crate::models::state::GatewayState;

#[derive(Debug, Serialize)]
struct JwtClaims<'a> {
    aud: &'a str,
    iat: i64,
    exp: i64,
}

pub struct MqttConnectOptions {
    pub trust_store_path: String,
    pub username: String,
    pub token_expiry_time_s: i64,
}

pub struct Mqtt {
    client: Client,
    rx: Option<Receiver<Option<Message>>>,
    gcp_project_id: String,
    gateway_id: String,
    connect_options: MqttConnectOptions,
}

impl<'a> Mqtt {
    pub fn new(rocket: &Rocket) -> Result<Self, paho_mqtt::Error> {
        let gcp_project_id = rocket.config().get_string("gcp_project_id").unwrap();
        let gcp_region = rocket.config().get_string("gcp_region").unwrap();
        let gcp_registry_name = rocket.config().get_string("gcp_registry_name").unwrap();
        let gateway_id = rocket.config().get_string("gateway_id").unwrap();
        let hostname = rocket.config().get_string("mqtt_hostname").unwrap();
        let trust_store_path = rocket.config().get_string("mqtt_trust_store_path").unwrap();
        let username = rocket.config().get_string("mqtt_username").unwrap();
        let token_expiry_time_s = rocket.config().get_int("mqtt_token_expiry_time_s").unwrap();
        
        let client_id = format!(
            "projects/{}/locations/{}/registries/{}/devices/{}",
            gcp_project_id,
            gcp_region,
            gcp_registry_name,
            gateway_id
        );

        let create_opts = CreateOptionsBuilder::new()
            .client_id(client_id)
            .server_uri(hostname)
            .finalize();

        let client = Client::new(create_opts)?;

        Ok(Mqtt {
            client,
            rx: None,
            gcp_project_id,
            gateway_id,
            connect_options: MqttConnectOptions {
                trust_store_path,
                username,
                token_expiry_time_s,
            }
        })
    }

    pub fn connect(
        &mut self,
    ) -> Result<(), jsonwebtoken::errors::Error> {
        let ssl_opts = SslOptionsBuilder::new()
            .trust_store(&self.connect_options.trust_store_path)
            .unwrap()
            .ssl_version(SslVersion::Tls_1_2)
            .verify(true)
            .disable_default_trust_store(true)
            .enable_server_cert_auth(true)
            .finalize();

        let conn_opts = ConnectOptionsBuilder::new()
            .ssl_options(ssl_opts)
            .user_name(&self.connect_options.username)
            .password(self.generate_password(self.connect_options.token_expiry_time_s)?)
            .finalize();

        self.client.connect(conn_opts).unwrap();

        self.rx = Some(self.client.start_consuming());

        Ok(())
    }

    pub fn subscribe_to_commands(&mut self, qos: i32) -> Result<(), paho_mqtt::Error> {
        let command_topic = format!("/devices/{}/commands/#", self.gateway_id);
        self.client.subscribe(&command_topic, qos)?;
        Ok(())
    }

    pub fn start_loop(
        self,
        gateway_state: Arc<Mutex<GatewayState>>,
        commands: Arc<Mutex<Vec<GatewayCommand>>>,
        discover: Arc<Mutex<bool>>,
    ) {
        let state_topic = format!("/devices/{}/state", self.gateway_id);
        let command_topic = format!("/devices/{}/commands", self.gateway_id);
        let discover_topic = format!("/devices/{}/commands/discover", self.gateway_id);

        loop {
            match self.rx.as_ref().unwrap().try_recv() {
                Ok(msg) => {
                    if let Some(msg) = msg {
                        let topic = msg.topic();

                        if topic == command_topic {
                            match serde_json::de::from_str(&*msg.payload_str()) {
                                Ok(gateway_command) => {
                                    commands.lock().unwrap().push(gateway_command);
                                }
                                Err(err) => {
                                    println!(
                                        "Gateway command deserialization failed with error: {}",
                                        err
                                    );
                                }
                            }
                        } else if topic == discover_topic {
                            println!("Got a discover request");
                            *discover.lock().unwrap() = true;
                        }
                    }
                }
                Err(TryRecvError::Empty) => {}
                Err(TryRecvError::Disconnected) => break,
            }

            let mut gateway_state_guard = gateway_state.lock().unwrap();

            if !gateway_state_guard.device_states.is_empty() {
                let content = serde_json::ser::to_string(&*gateway_state_guard).unwrap();
                let message = Message::new(&state_topic, content, 1);

                self.client.publish(message).unwrap();

                gateway_state_guard.device_states.clear();
                drop(gateway_state_guard);

                thread::sleep(Duration::from_secs(1));
            }
        }
    }

    fn generate_password(
        &self,
        token_expiry_time_s: i64,
    ) -> Result<String, jsonwebtoken::errors::Error> {
        let now = Utc::now().timestamp();

        let my_claims = JwtClaims {
            aud: &self.gcp_project_id,
            iat: now,
            exp: now + token_expiry_time_s,
        };

        let f = File::open("./rsa_private.pem").unwrap();
        let mut reader = BufReader::new(f);
        let mut private_key = Vec::new();
        reader.read_to_end(&mut private_key).unwrap();

        encode(
            &Header::new(Algorithm::RS256),
            &my_claims,
            &EncodingKey::from_rsa_pem(&private_key).unwrap(),
        )
    }
}
